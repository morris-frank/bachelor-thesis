...
def integralRect(ii, start, end):
    '''Returns the sum inside a square from an integral image.

    Args:
        ii (ndarray): The integral image
        start (tuple): the top left point
        end (tuple): the bottom right point
    '''
    return ii[end[0], end[1]] + ii[start[0], start[1]] - ii[start[0], end[1]] - ii[end[0], start[1]]
...
def scoreToRegion(heatmap, image):
    ...
    Returns:
        The maximum bounding box (x, y, w, h)
    '''
    labels, regions = selective_search(image, scale=500, sigma=0.9, min_size=10)
    starts = []
    ends = []
    ...
    #     if end[0] >= heatmap.shape[0]:
    #         ends[i] = (heatmap.shape[0] - 1, ends[i][1])
    #     if end[1] >= heatmap.shape[1]:
    #         ends[i] = (ends[i][0], heatmap.shape[1] - 1)

    # Not working at the moment...
    bbscores = skimage.transform.integrate(integral_scores, starts, ends)
    bbidx = bbscores.index(max(bbscores))
    x, y = starts[bbidx]
    x2, y2 = ends[bbidx]
    regmax = (x, y, x2 - x, y2 - y)

    # curmax = 0
    # regmax = ()
    # for start, end in zip(starts, ends):
    #     bsum = integralRect(integral_scores, start, end)
    #     if bsum > curmax:
    #         curmax = bsum
    #         regmax = (start[0], start[1], end[0] - start[0], end[1] - start[1])
    return regmax
