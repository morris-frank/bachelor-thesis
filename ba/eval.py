...
def _generic_box(image, gridwidth=7):
    '''Returns a generic grid of boxes for an image. A little bit like done
    on YOLO

    Args:
        image (ndarray): The image
        gridwidth (int, optional): The number of boxes per dimension

    Returns:
        a list of regions
    '''
    widths = (np.ceil(image.shape[0] / gridwidth),
              np.ceil(image.shape[1] / gridwidth))
    starts = np.ogrid[0:image.shape[0] - widths[0]:gridwidth + 1j,
                      widths[0]:image.shape[0]:gridwidth + 1j]
    ends = np.ogrid[0:image.shape[1] - widths[1]:gridwidth + 1j,
                    widths[1]:image.shape[1]:gridwidth + 1j]
    regions = []
    for x1, y1 in zip(starts[0].flat, starts[1].flat):
        for x2, y2 in zip(ends[0].flat, ends[1].flat):
            regions.append((0, (x1, y1, x2, y2)))
    return regions
